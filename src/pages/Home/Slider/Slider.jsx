...
const Slider = ({ slides }) => {
    ...
    return (
        ...
        <div className="flex flex-col md:flex-row">
            <img
                className={`w-full md:w-1/2 h-auto ${animateSlide ? 'slider-animation' : ''}`}
                src={slides[currentSlide].imgLeft}
                alt=""
                onAnimationEnd={handleAnimationEnd} // Reset animation state when animation ends
            />
            <div
                className={`w-full py-10 md:w-1/2 h-auto flex flex-col items-center justify-center ${animateSlide ? 'slider-animation' : ''}`}
                style={slideRightStyle}
                onAnimationEnd={handleAnimationEnd} // Reset animation state when animation ends
            >
        <div className="max-w-[300px] max-h-[100px] mb-8">
          <img className="w-full h-full max-h-[100px]" src={slides[currentSlide].logo} alt="" />
        </div>
    ...
    <div className="flex gap-1 items-center justify-center my-5">
                {slides.map((_, index) => (
                    <span
                        key={index}
                        className={`w-3 h-3 rounded-full ${index === currentSlide ? 'bg-purple' : 'bg-black'}`}
